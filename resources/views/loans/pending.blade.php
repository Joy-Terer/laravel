...
<div class="row g-3 mb-4">
    ...
    <div class="col-sm-4">
        <div class="stat-card">
            <div class="stat-icon bg-danger-light"><i class="bi bi-hourglass-split text-danger-custom"></i></div>
            <div class="stat-label">Outstanding Balance</div>
            <div class="stat-value">KES {{ number_format($loans->whereIn('status',['approved','active'])->sum('balance'), 0) }}</div>
        </div>
    </div>
    <div class="col-sm-4">
        <div class="stat-card">
            <div class="stat-icon bg-success-light"><i class="bi bi-check-circle text-success-custom"></i></div>
    ...
    <div class="card-body-custom">
                ...
                <div class="loan-card-header">
                    <div>
                        ...
                        <div class="loan-card-meta">
                            {{ $loan->purpose }} · {{ $loan->repayment_period }} month(s) · Applied {{ $loan->created_at->format('d M Y') }}
                        </div>
                    ...
                    <div class="loan-card-status">
                        @if($loan->status==='pending')   <span class="badge-custom badge-warning"><i class="bi bi-clock"></i> Pending</span>
                        @elseif($loan->status==='approved') <span class="badge-custom badge-primary"><i class="bi bi-check-circle"></i> Approved</span>
                        @elseif($loan->status==='repaid') <span class="badge-custom badge-success"><i class="bi bi-check-circle-fill"></i> Repaid</span>
                        @else <span class="badge-custom badge-danger"><i class="bi bi-x-circle"></i> Declined</span>
                        @endif
                    </div>
                </div>
                ...
                @if(in_array($loan->status, ['approved','active']))
                    ...
                    <div class="loan-progress-section">
                        <div class="loan-progress-labels">
                            <span>Balance: KES {{ number_format($loan->balance, 0) }}</span>
                        ...
                        <form method="POST" action="{{ route('loans.repay', $loan->id) }}" class="repay-form">
                            ...
                            <div>
                                ...
                                <input type="number" name="amount" class="form-control-custom"
                                       placeholder="e.g. 1000" min="1" max="{{ $loan->balance }}" required>
                            </div>
                            <div>
        ...
        @empty
            <div class="empty-state">
                ...
                <a href="{{ route('loans.apply') }}" class="btn-primary-custom" style="margin-top:14px">Apply for a Loan</a>
            </div>
        @endforelse
    </div>
</div>
